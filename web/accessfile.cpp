...
bool MatchAddresses(const HttpServer::Sockaddr& inAddr1, const HttpServer::Sockaddr& inAddr2,
                    const HttpServer::Sockaddr& inMask) {
  if (inAddr1.sa.sa_family != inAddr2.sa.sa_family) return false;
  ...
  for (int i = 0; i < width / 8; ++i) {
    char c1 = data1[i] & maskdata[i], c2 = data2[i] & maskdata[i];
...
AccessFile::AccessFile(const std::string& path) {
  ...
  while (std::getline(file, line)) {
    while (!line.empty() && std::iswspace(line.front())) line = line.substr(1);
...
const std::string& AccessFile::errors() const { return mErrors; }
...
bool AccessFile::isAllowed(const HttpServer::Sockaddr& addr) const {
  ...
  for (const auto& entry : mEntries) {
    ...
    if (result == Entry::Allow) return true;
    if (result == Entry::Deny) return false;
  }
...
int AccessFile::Entry::match(const HttpServer::Sockaddr& addr) const {
  for (const auto& network : mNetworks) {
    if (MatchAddresses(addr, network.address, network.mask)) {
      std::clog << ((mKind == Allow) ? "allowing " : "denying ") << HttpServer::ipString(addr)
                << ", matching rule: " << mRule << std::endl;
      return mKind;
    }
  }
  return NoMatch;
