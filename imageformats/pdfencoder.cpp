...
std::ostream& operator<<(std::ostream& os, const smanip& s) {
  s.insert(os);
  return os;
}

std::string pdfNameEncode(const std::string& s) {
  std::string r;
...
std::string pdfStringEncode(const std::string& s) {
  std::string r;
  for (const auto c : s) switch (c) {
      case '\n':
        r += "\\n";
...
struct PdfEncoder::Private {
  ...
  struct objdef {
    ...
    static bool less(const objdef& obj1, const objdef& obj2) { return obj1.id < obj2.id; }
  };
...
PdfEncoder::Private::insert_objdef PdfEncoder::Private::defobj(int id) {
  return insert_objdef(this, id);
}

PdfEncoder::Private::Private(PdfEncoder* p) : p(p), mBegin(0), mStartxref(0) {}

PdfEncoder::PdfEncoder() : p(new Private(this)) {}

PdfEncoder::~PdfEncoder() { delete p; }

Dictionary& PdfEncoder::documentInfo() { return p->mInfoDict; }

const Dictionary& PdfEncoder::documentInfo() const { return p->mInfoDict; }

void PdfEncoder::onDocumentBegin() {
#if BYTE_ORDER == LITTLE_ENDIAN
  p->mLineBuffer.clear();
  if (bitDepth() == 16) p->mLineBuffer.resize(components() * width());
#endif
  ...
  for (const auto& entry : p->mInfoDict)
    if (!entry.first.empty() && !entry.second.empty())
      p->mInfoString +=
          "/" + pdfNameEncode(entry.first) + " (" + pdfStringEncode(entry.second) + ")\n";

  p->mObjects.clear();
  if (resolutionDpi() == 0) throw std::runtime_error("no resolution specified");
...
void PdfEncoder::onImageBegin() {
  ...
  *destination() << p->defobj(++p->mObj)  // mObj == 2
                 ...
                 << "/MediaBox [ 0 0 " << pdfunits_per_px * width() << " "
...
void PdfEncoder::onImageEnd() {
  const double pdfunits_per_px = 72.0 / resolutionDpi();
  std::ostringstream oss;
  oss << pdfunits_per_px * width() << " 0 0 " << pdfunits_per_px * height() << " 0 0 cm\n"
      << "/strip0 Do\n";
...
void PdfEncoder::onDocumentEnd() {
  ...
  os << p->defobj(1) << "\n<<\n"
     ...
     << "/Count " << currentImage() << "\n"
     << "/Kids [\n";
  for (int i = 2; i < p->mObj; i += 3) os << i << " 0 R\n";
  os << "\n]\n>>\nendobj\n";
...
void PdfEncoder::onWriteLine(const void* pReadFrom) {
#if BYTE_ORDER == LITTLE_ENDIAN
  if (!p->mLineBuffer.empty()) {
    ...
    while (pIn < pEnd) *pOut++ = htons(*pIn++);
    destination()->write(reinterpret_cast<const char*>(p->mLineBuffer.data()), bytesPerLine());
    return;
  }
#endif
...
PdfEncoder::Private::insert_objdef::insert_objdef(Private* p, int id) : p(p), id(id) {}

void PdfEncoder::Private::insert_objdef::insert(std::ostream& os) const {
