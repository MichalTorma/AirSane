...
struct NetworkHotplugNotifier::Private {
  ...
  Private(NetworkHotplugNotifier* pNotifier)
    ...
    int fds[2];
    if (::pipe(fds) < 0) {
      std::cerr << "Could not create socket pair " << ::strerror(errno) << std::endl;
      return;
    }
    mPipeReadFd = fds[0];
    mPipeWriteFd = fds[1];
  ...
  void initAddresses() {
    ...
    for (ifaddrs* p = pAddrs; p != nullptr; p = p->ifa_next) {
      ...
      if (address.sa.sa_family != AF_UNSPEC) mAddresses.insert(address);
    }
    ::freeifaddrs(pAddrs);
  }
  ...
  void hotplugThread() {
    ...
    if (sock < 0) {
      std::cerr << "Could not create netlink socket: " << ::strerror(errno) << std::endl;
      return;
    }
    ...
    while (!done) {
      ...
      if (r > 0 && pfds[0].revents) {
        ...
        if (len > 0) {
          ...
          if (data.n->nlmsg_flags & MSG_TRUNC) continue;
          while (NLMSG_OK(data.n, len) && (data.n->nlmsg_type != NLMSG_DONE)) {
            if (data.n->nlmsg_type == RTM_NEWADDR || data.n->nlmsg_type == RTM_DELADDR ||
