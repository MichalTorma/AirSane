...
#undef _
    {0, nullptr}};
const char* dnssd_strerr(int code) {
  for (const auto err : sErrors)
    if (err.code == code) return err.text;
...
struct ServiceEntry {
  ...
  ServiceEntry(MdnsPublisher::Service* p) : mpService(p), mDNSServiceRef(nullptr) {}
  ...
  bool announce() {
    ...
    for (const auto& entry : mpService->txtRecord()) {
      DNSServiceErrorType err = ::TXTRecordSetValue(&txtRecord, entry.first.c_str(),
                                                    entry.second.length(), entry.second.data());
      if (err != kDNSServiceErr_NoError) {
        ok = false;
        std::cerr << "Could not add txtRecord value " << entry.first << "=" << entry.second << " ("
                  << dnssd_strerr(err) << ")" << std::endl;
      }
    }
    int ifindex = mpService->interfaceIndex();
    if (ifindex < 0) ifindex = 0;
    uint16_t port = mpService->port();
    DNSServiceErrorType err = ::DNSServiceRegister(
        &mDNSServiceRef, 0, ifindex, mpService->name().c_str(), mpService->type().c_str(), nullptr,
        nullptr, htons(port), ::TXTRecordGetLength(&txtRecord), ::TXTRecordGetBytesPtr(&txtRecord),
        onRegisterService, mpService);
    if (err != kDNSServiceErr_NoError) {
      ok = false;
      std::cerr << "Could not register service " << mpService->name() << " (" << dnssd_strerr(err)
                << ")" << std::endl;
    } else {
      ...
      if (r == 1) ::DNSServiceProcessResult(mDNSServiceRef);
    }
  ...
  static void onRegisterService(DNSServiceRef, DNSServiceFlags, DNSServiceErrorType errorCode,
                                const char* name, const char* regType, const char* domain,
                                void* data) {
    auto* pService = static_cast<MdnsPublisher::Service*>(data);
    if (errorCode == kDNSServiceErr_NoError) pService->setName(name);
  }
};
...
MdnsPublisher::MdnsPublisher() : p(new Private) {
  ...
  if ([names count] > 0) p->mHostnameFqdn = [names[0] UTF8String];
